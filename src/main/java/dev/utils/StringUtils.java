...
public final class StringUtils {
	...
	public static int levenshteinDistance(String lhs, String rhs) {
		...
		String lhsNWS = lhs.toLowerCase().trim();
		String rhsNWS = rhs.toLowerCase().trim();
		
		int len0 = lhsNWS.length() + 1;
		int len1 = rhsNWS.length() + 1;
		...
		for (int j = 1; j < len1; j++) {
			...
			for (int i = 1; i < len0; i++) {
				int match = (lhsNWS.charAt(i - 1) == rhsNWS.charAt(j - 1)) ? 0 : 1;
